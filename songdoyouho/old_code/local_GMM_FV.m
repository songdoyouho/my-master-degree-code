...
for j=1:actnum
    for i=1:numel(tridx{j,1})
        vi=tridx{j,1}(1,i);
        % load features
        load(sprintf('%s\\output_global_and_local_traj\\feature%d_%d.mat', conf.videopath, vi, j));
        fprintf('%s\\output_global_and_local_traj\\feature%d_%d.mat\n', conf.videopath, vi, j);
        XXX = [XXX; X];
    end
end
...
save(sprintf('%s\\local_coeff1.mat',conf.modelpath),'newcoeff1');
save(sprintf('%s\\local_coeff2.mat',conf.modelpath),'newcoeff2');
save(sprintf('%s\\local_coeff3.mat',conf.modelpath),'newcoeff3');
save(sprintf('%s\\local_coeff4.mat',conf.modelpath),'newcoeff4');
save(sprintf('%s\\local_coeff5.mat',conf.modelpath),'newcoeff5');
...
%save GMM model
save(sprintf('%s\\local_model1.mat',conf.modelpath),'model1');
save(sprintf('%s\\local_model2.mat',conf.modelpath),'model2');
save(sprintf('%s\\local_model3.mat',conf.modelpath),'model3');
save(sprintf('%s\\local_model4.mat',conf.modelpath),'model4');
save(sprintf('%s\\local_model5.mat',conf.modelpath),'model5');
...
for j=1:actnum
    for i=1:numel(tridx{j,1})
        ...
        % save fisher vector
        save(sprintf('%s\\encoding\\local_encoding%d_%d.mat',conf.tmppath,vi,j),'encoding');
    end
end
