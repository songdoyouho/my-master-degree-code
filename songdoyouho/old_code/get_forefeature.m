%NEWGETFISHER Summary of this function goes here
...
%         vi=tridx{j,1}(1,i);
%         % load features
%         load(sprintf('%s\\fore_feature\\fore_feature%d_%d.mat', conf.tmppath, vi, j));
%         fprintf('%s\\fore_feature\\fore_feature%d_%d.mat\n', conf.tmppath, vi, j);
%         % select feature for training GMM
%         if size(fore_feature,1)<featurenum
%             XXX=[XXX; fore_feature];
%         else
%             idid=randperm(size(fore_feature,1),featurenum);
%             X_idx = 1:size(fore_feature,1);
%             idx = ismember(X_idx,idid);
%             idx=idx';
%             fore_feature = fore_feature(idx,:);
%             XXX=[XXX; fore_feature];
%         end
%     end
% end
...
% pcaX5=X5*newcoeff5;
% 
% save(sprintf('%s\\fore_coeff1.mat',conf.modelpath),'newcoeff1');
% save(sprintf('%s\\fore_coeff2.mat',conf.modelpath),'newcoeff2');
% save(sprintf('%s\\fore_coeff3.mat',conf.modelpath),'newcoeff3');
% save(sprintf('%s\\fore_coeff4.mat',conf.modelpath),'newcoeff4');
% save(sprintf('%s\\fore_coeff5.mat',conf.modelpath),'newcoeff5');
% 
% % train GMM model
...
% 
% %save GMM model
% save(sprintf('%s\\fore_model1.mat',conf.modelpath),'model1');
% save(sprintf('%s\\fore_model2.mat',conf.modelpath),'model2');
% save(sprintf('%s\\fore_model3.mat',conf.modelpath),'model3');
% save(sprintf('%s\\fore_model4.mat',conf.modelpath),'model4');
% save(sprintf('%s\\fore_model5.mat',conf.modelpath),'model5');
...
for j=1:actnum
    for i=1:numel(tridx{j,1})
        ...
        load(sprintf('%s\\fore_feature\\fore_feature%d_%d.mat', conf.tmppath, vi, j));
        fprintf('%s\\fore_feature\\fore_feature%d_%d.mat\n', conf.tmppath, vi, j); 
        X1=fore_feature(:,2:31);
        X2=fore_feature(:,32:127);
        X3=fore_feature(:,128:235);
        X4=fore_feature(:,236:331);
        X5=fore_feature(:,332:427);
        
        % pca
        load(sprintf('%s\\fore_coeff1.mat',conf.modelpath));
        pcaX1=X1*newcoeff1;
        load(sprintf('%s\\fore_coeff2.mat',conf.modelpath));
        pcaX2=X2*newcoeff2;
        load(sprintf('%s\\fore_coeff3.mat',conf.modelpath));
        pcaX3=X3*newcoeff3;
        load(sprintf('%s\\fore_coeff4.mat',conf.modelpath));
        pcaX4=X4*newcoeff4;
        load(sprintf('%s\\fore_coeff5.mat',conf.modelpath));
        pcaX5=X5*newcoeff5;
        ...
        % save fisher vector
        save(sprintf('%s\\encoding\\fore_encoding%d_%d.mat',conf.tmppath,vi,j),'encoding');
    end
end
% turn into fisher vector
for j=1:actnum
