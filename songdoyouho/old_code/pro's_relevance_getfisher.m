...
for j=1:actnum
    for i=1:numel(tridx{j,1})
        vi=tridx{j,1}(1,i);
        % load features
        load(sprintf('%s\\relevance_feature\\relevance_feature%d_%d.mat', conf.videopath, vi, j));
        fprintf('%s\\relevance_feature\\relevance_feature%d_%d.mat\n', conf.videopath, vi, j);
        % select feature for training GMM
        if size(relevance_feature,1)<featurenum
            XXX=[XXX; relevance_feature];
        else
            idid=randperm(size(relevance_feature,1),featurenum);
            X_idx = 1:size(relevance_feature,1);
            idx = ismember(X_idx,idid);
            idx=idx';
            relevance_feature = relevance_feature(idx,:);
            XXX=[XXX; relevance_feature];
        end
    end
end
...
save(sprintf('%s\\relevance_coeff1.mat',conf.modelpath),'newcoeff1');
save(sprintf('%s\\relevance_coeff2.mat',conf.modelpath),'newcoeff2');
save(sprintf('%s\\relevance_coeff3.mat',conf.modelpath),'newcoeff3');
save(sprintf('%s\\relevance_coeff4.mat',conf.modelpath),'newcoeff4');
save(sprintf('%s\\relevance_coeff5.mat',conf.modelpath),'newcoeff5');
...
%save GMM model
save(sprintf('%s\\relevance_model1.mat',conf.modelpath),'model1');
save(sprintf('%s\\relevance_model2.mat',conf.modelpath),'model2');
save(sprintf('%s\\relevance_model3.mat',conf.modelpath),'model3');
save(sprintf('%s\\relevance_model4.mat',conf.modelpath),'model4');
save(sprintf('%s\\relevance_model5.mat',conf.modelpath),'model5');
...
for j=1:actnum
    for i=1:numel(tridx{j,1})
        ...
        % save fisher vector
        save(sprintf('%s\\relevance_encoding\\encoding%d_%d.mat',conf.tmppath,vi,j),'encoding');
    end
end
