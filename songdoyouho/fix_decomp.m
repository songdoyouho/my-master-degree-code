...
%         mkdir(sprintf('%s\\new_decompose_traj\\video_TrajAc\\%d_%d',conf.videopath,vi,j));
        mkdir(sprintf('%s\\fix_decompose_traj\\video_TrajEt\\%d_%d',conf.videopath,vi,j));
        ...
        outputvideo = VideoWriter(sprintf('%s\\fix_decompose_traj\\fixkmean%d_%d.avi',conf.videopath,vi,j));
        ...
        for k = 1 : interval_num : endframe - trajlength
            thisframe=read(mov,k + trajlength);
...
%             U = traj(1:100:end,1:2:21);
%             V = traj(1:100:end,2:2:22);
%             line(U',V','Color','green');
%             text(1, 5, 'traj');
%             image1 = getframe(gcf);
%             clf

            plot(video_TrajEt(1:10:end,1),video_TrajEt(1:10:end,trajlength + 1),'r*'); hold on
            U = video_TrajEt(1:10:end,1:trajlength);
            V = video_TrajEt(1:10:end,trajlength + 1 : trajlength * 2);
            line(U',V','Color','green');
            text(1, 5, 'Et');
            image = getframe(gcf);
            clf            
...
%             save(sprintf('%s\\new_decompose_traj\\video_TrajOut\\%d_%d\\%d.mat',conf.videopath,vi,j,k+trajlength),'video_TrajOut');
%                       
%             time_stamp = zeros(size(video_TrajIn,1),1);
%             time_stamp(:,1) = k;
%             video_TrajIn = [video_TrajIn time_stamp];
%             video_TrajIn = video_TrajIn(1:100:end,:);
%             save(sprintf('%s\\new_decompose_traj\\video_TrajIn\\%d_%d\\%d.mat',conf.videopath,vi,j,k+trajlength),'video_TrajIn');
%                          
%             time_stamp = zeros(size(video_TrajAc,1),1);
%             time_stamp(:,1) = k;
%             video_TrajAc = [video_TrajAc time_stamp];
%             video_TrajAc = video_TrajAc(1:100:end,:);
%             save(sprintf('%s\\new_decompose_traj\\video_TrajAc\\%d_%d\\%d.mat',conf.videopath,vi,j,k+trajlength),'video_TrajAc');
                       
            time_stamp = zeros(size(video_TrajEt,1),1);
            time_stamp(:,1) = k;
            video_TrajEt = [video_TrajEt time_stamp];
            save(sprintf('%s\\fix_decompose_traj\\video_TrajEt\\%d_%d\\%d.mat',conf.videopath,vi,j,k+trajlength),'video_TrajEt');
        end 
        close(outputvideo); 
    end
end
