function [label, center, bCon, sumD, D] = paperkmeans(X, k, varargin)
...
bigdelta_Tx = repmat(delta_Tx',k,1)';
bigdelta_Ty = repmat(delta_Ty',k,1)';
    ...
    switch distance
        case 'sqeuclidean'
            while any(label ~= last) && it<maxit
                ...
                delta_Cx = center(:,16) - center(:,1);
                delta_Cy = center(:,32) - center(:,17);
                
                bigdelta_Cx = repmat(delta_Cx',size(delta_Tx,1),1);
                bigdelta_Cy = repmat(delta_Cy',size(delta_Ty,1),1);
                
                D = sqrt((bigdelta_Tx - bigdelta_Cx).*(bigdelta_Tx - bigdelta_Cx) + (bigdelta_Ty - bigdelta_Cy).*(bigdelta_Ty - bigdelta_Cy));
