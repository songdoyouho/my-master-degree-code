...
mexFunction (int nout VL_UNUSED, mxArray * out[], int nin, const mxArray * in[])
{
  ...
  OUT(ENC) = mxCreateNumericMatrix (numClusters, numData, classID, mxREAL) ;

  if (dataType == VL_TYPE_FLOAT)
  	vl_get_gmm_data_posteriors_f (mxGetData(OUT(ENC)), numClusters, numData, 
		  priors, means, dimension, covariances, data);
  else vl_get_gmm_data_posteriors_d (mxGetData(OUT(ENC)), numClusters, numData, 
		  priors, means, dimension, covariances, data);
	  /*
